<template>
    <AdminLayout>
        ...
        <cs-form-section @submitted="update">
            <template #title> Update Course </template>
...
export default {
    ...
    mounted() {
        ...
        for (let index = 0; index < this.course.topics.length; index++) {
            const topic = this.course.topics[index];
    ...
    methods: {
        update() {
            this.form.put(
                route("admin.courses.update", { id: this.course.id }),
                {
                    preserveScroll: true,
                    preserveState: true,
